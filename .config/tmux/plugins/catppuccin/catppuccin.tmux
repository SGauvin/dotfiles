...
build_window_format() {
  ...
  then
    ...
    local show_middle_separator="#[fg=$background,bg=$thm_bg,nobold,nounderscore,noitalics]$window_middle_separator"
    local show_text="#[fg=$background,bg=$thm_bg]$text"
...
main() {
  ...
  # status
  set status "on"
  set status-bg default  # default: set status-bg "${thm_bg}"
